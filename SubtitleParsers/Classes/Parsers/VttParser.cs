namespace SubtitleParsers.Classes.Parsers
{
    ...
    /// <summary>
    ///     Parser for the .vtt subtitles files. Does not handle formatting tags within the text; that has to be parsed
    ///     separately.
    ...
    public class VttParser : ISubtitlesParser
    {
        ...
        public List<SubtitleItem> ParseStream(Stream vttStream, Encoding encoding)
        {
            ...
            if (vttSubParts.Any())
            {
                ...
                {
                    var lines = vttSubPart.Split(new[] { Environment.NewLine }, StringSplitOptions.None)
                        .Select(s => s.Trim()).Where(l => !string.IsNullOrEmpty(l)).ToList();
                    ...
                    if ((item.StartTime != 0 || item.EndTime != 0) && item.Lines.Any()) items.Add(item);
                }
