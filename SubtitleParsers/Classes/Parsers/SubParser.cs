namespace SubtitleParsers.Classes.Parsers
{
    ...
    {
        // Properties -----------------------------------------------------------------------
        private readonly Dictionary<SubtitlesFormat, ISubtitlesParser> _subFormatToParser =
            new Dictionary<SubtitlesFormat, ISubtitlesParser>
                {
        ...
        public SubtitlesFormat GetMostLikelyFormat(string fileName)
        {
            ...
            if (!string.IsNullOrEmpty(extension))
                foreach (var format in SubtitlesFormat.SupportedSubtitlesFormats)
                    if (format.Extension != null && Regex.IsMatch(
                            extension,
                            format.Extension,
                            RegexOptions.IgnoreCase)) return format;
        ...
        ///     The preferred subFormat to try first (if we have a clue with the subtitle file name for
        ///     example)
        /// </param>
        ...
        public List<SubtitleItem> ParseStream(Stream stream, Encoding encoding, SubtitlesFormat subFormat = null)
        {
            var dictionary = subFormat != null
                                 ? _subFormatToParser
                                     ...
                                     .OrderBy(
                                         dic => Math.Abs(
                                             string.Compare(dic.Key.Name, subFormat.Name, StringComparison.Ordinal)))
        ...
        /// <returns>The corresponding list of SubtitleItem, null if parsing failed</returns>
        public List<SubtitleItem> ParseStream(
            Stream stream,
            Encoding encoding,
            Dictionary<SubtitlesFormat, ISubtitlesParser> subFormatDictionary)
        {
            // test if stream if readable
            if (!stream.CanRead) throw new ArgumentException("Cannot parse a non-readable stream");
            ...
            var message = string.Format(
                "All the subtitles parsers failed to parse the following stream:{0}",
                firstCharsOfFile);
        ...
        private string LogFirstCharactersOfStream(Stream stream, int nbOfCharactersToPrint, Encoding encoding)
        {
            ...
            if (stream.CanRead)
            {
                ...
                message = string.Format(
                    "Parsing of subtitle stream failed. Beginning of sub stream:\n{0}",
                    string.Join(string.Empty, buffer));
            }
            else
            {
                message = string.Format(
                    "Tried to log the first {0} characters of a closed stream",
